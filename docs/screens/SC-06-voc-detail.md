...
| 버튼 | 타입 | 설명 | 표시 조건 | 비활성화 조건 |
|------|------|------|----------|--------------|
| 이메일 발송 | 버튼 (보조) | 최종 사용자에게 이메일 발송 | 항상 | - |
| 반려 | 버튼 (위험) | VOC 반려 처리 | 항상 | **종료 상태(완료/반려)인 경우** |
| 완료 처리 | 버튼 (주요) | VOC 완료 처리 | 항상 | **종료 상태(완료/반려)인 경우** |

**종료 상태 (Terminal Status)**:
- 완료(RESOLVED), 반려(REJECTED), 종료(CLOSED)는 종료 상태입니다.
- 종료 상태에서는 반려/완료 처리 버튼이 비활성화됩니다 (`disabled`, `aria-disabled` 속성 적용).
- 이메일 발송은 종료 상태에서도 가능합니다.
...
| 1.1 | 2026-02-04 | Claude Code | 하단 버튼 순서 변경 (이메일 발송 → 반려 → 완료 처리), 종료 상태 개념 추가 |
