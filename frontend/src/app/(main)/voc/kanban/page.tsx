      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {STATUS_COLUMNS.map((column) => {
          ...
          return (
            <div
              key={column.status}
              className="flex-shrink-0 w-72 bg-gray-100 dark:bg-gray-800 rounded-lg"
            >
              {/* Column Header */}
                ...
                {columnVocs.length === 0 ? (
                  ...
                  columnVocs.map((voc) => (
                    <Link
                      key={voc.id}
                      href={`/voc/${voc.id}`}
                      className="block p-3 bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow cursor-pointer"
                      draggable
                      onDragStart={() => setDraggedId(voc.id)}
